...
[source,c#,indent=0]
----
include::starter_complete/Starter/Starter/Global.asax.cs[tag=Application_Start]
----

You'll also need to specify credentials. Starting in Couchbase 5, buckets no longer have passwords. Instead, you can create a "user" in Couchbase and give it access to buckets via roles. Here, I'm creating a user with full bucket access permission:
...
For this guide, I'm going to skip talking about `ScanConsistency`. There are trade-offs to consider when using different ScanConsistency options. However, RequestPlus is the easiest to use for a simple guide like this one.
...
The sample code has a WebAPI controller `HomeController` already set up with some Actions and Views. The actions use the repository you created and pass along objects to the views. Views. The actions use the repository you created and pass along objects to the views.
...
Now, pick an ID from one of these results to make changes to. Make some changes to the fields (for instance, change the firstName) and POST to the Save endpoint.

image:image004-save.png[Update profile via API using Postman]

We've covered Read, Create, Update, so all that's left is delete. Pick an ID from one of the documents to delete and use Postman to create a DELETE request.
